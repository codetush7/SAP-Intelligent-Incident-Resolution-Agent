...
async function getCategoryBreakdown(userId) {
  const tickets = await dataStore.getTickets(userId);
  const categories = {};
  tickets.forEach(t => {
...
async function getStats(req, res) {
  ...
  const monitoring = await getMonitoringStatus(userId);
  const activeAlerts = (await dataStore.getAlerts(userId)).filter(a => !a.acknowledged).length;

  const recentIssues = (await dataStore.getTickets(userId))
  ...
  res.json({
    ...
    categories: await getCategoryBreakdown(userId),
    recentActivity: (await dataStore.getMonitoringLogs(userId)).slice(0, 5),
    recentIssues,
