...
const signupSchema = Joi.object({
  ...
  lastName: Joi.string().trim().max(60).optional().allow(''),
...
async function signup(req, res) {
  ...
  try {
    const user = await userStore.create({ ...value, name });
...
async function updateProfile(req, res) {
  ...
  try {
    const updated = await userStore.updateProfile(req.user.id, value);
