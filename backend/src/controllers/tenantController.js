...
async function listTenants(req, res) {
  res.json({ tenants: await tenantStore.getAll(req.user.id) });
}
...
async function createTenant(req, res) {
  ...
  try {
    ...
    return res.status(201).json({ tenant: await tenantStore.getByIdPublic(tenant.id, req.user.id), test: result });
  } catch (err) {
    ...
    return res.status(201).json({
      tenant: await tenantStore.getByIdPublic(tenant.id, req.user.id),
...
async function retestTenant(req, res) {
  ...
  try {
    ...
    return res.json({ tenant: await tenantStore.getByIdPublic(creds.id, req.user.id), test: result });
  } catch (err) {
    const message = err.response?.data?.error_description || err.response?.data?.error || err.message;
    await tenantStore.setStatus(creds.id, req.user.id, 'FAILED', message);
    return res.status(200).json({
      tenant: await tenantStore.getByIdPublic(creds.id, req.user.id),
      test: { tokenObtained: false, error: message }
    });
  }
}
...
async function updateTenant(req, res) {
  ...
  res.json({ tenant: await tenantStore.getByIdPublic(tenant.id, req.user.id) });
}
...
async function activateTenant(req, res) {
  ...
  res.json({ tenant: await tenantStore.getByIdPublic(req.params.id, req.user.id) });
}
