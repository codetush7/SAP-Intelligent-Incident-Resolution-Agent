  ...
  try {
    ...
    for (const cert of certs) {
      ...
      if (daysLeft <= warningDays && daysLeft >= 0) {
        const existing = (await dataStore.getTickets(userId)).find(t => t.certName === cert.Hexalias);
...
async function checkIFlowStatus(userId) {
  ...
  try {
    ...
    const existing = (await dataStore.getTickets(userId)).find(t => t.iflow === errored.Name && t.errorCode === 'HTTP_500');
...
async function getMonitoringStatus(userId) {
  return {
    ...
    mode: (await isSAPConfigured(userId)) ? 'LIVE' : 'SIMULATION',
    sapConfigured: await isSAPConfigured(userId),
