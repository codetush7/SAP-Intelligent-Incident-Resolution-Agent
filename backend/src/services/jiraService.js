...
async function resolveIssueType(client, projectKey) {
  ...
  try {
    const meta = await client.get(`/issue/createmeta?projectKeys=${projectKey}`);
    const proj = meta.data?.projects?.[0];
    const types = (proj?.issuetypes || []).filter(t => !t.subtask);
    if (types.length > 0) {
      const preferred = ['Task', 'Incident', 'Bug', 'Story', 'Request'];
      for (const pref of preferred) {
        const found = types.find(t => t.name.toLowerCase() === pref.toLowerCase());
        if (found) {
          issueTypeCache.set(projectKey, found.name);
          return found.name;
        }
      }
      issueTypeCache.set(projectKey, types[0].name);
      return types[0].name;
    }
  } catch (err) {
    logger.warn(`[Jira] Could not fetch createmeta for ${projectKey}: ${err.message}`);
  }
  return 'Task';
}
...
async function createJiraIssue(creds, ticketData) {
  ...
  const issueTypeName = await resolveIssueType(client, creds.projectKey);

  const cleanCategory = String(ticketData.category || 'GENERAL').replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_\-]/g, '');

  const body = {
    fields: {
      project: { key: creds.projectKey },
      summary: ticketData.title || `CPI Incident: ${ticketData.ticketNumber || 'Integration Error'}`,
      description: {
        type: 'doc',
        version: 1,
        content: [{
      ...
      labels: ['SAP-CPI', 'AI-Auto-Created', cleanCategory]
    }
  };
