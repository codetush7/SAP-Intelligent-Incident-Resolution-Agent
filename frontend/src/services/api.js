...
api.interceptors.request.use(config => {
  ...
  // Only inject real JWTs — skip the frontend-only demo-token
...
export const agentAPI = {
  // provider/model are optional overrides — e.g. { provider: 'gemini', model: 'gemini-2.0-flash' }
  // from the AI Agent page's dropdown. Omit to use whatever is active in Settings > AI Configuration.
  chat: (messages, providerOverride) => api.post('/agent/chat', { messages, ...(providerOverride || {}) }),
  ...
  getAiConfig: () => api.get('/agent/ai-config'),
  saveAiConfig: (config) => api.post('/agent/ai-config', config)
};
