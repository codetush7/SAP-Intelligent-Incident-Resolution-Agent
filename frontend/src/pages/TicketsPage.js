...
import { SeverityBadge, CategoryChip, FilterSelect } from '../components/common';

const STATUS_COLOR = {
  Open: 'var(--sap-high)',
  'In Progress': 'var(--sap-info)',
  Resolved: 'var(--sap-success)',
...
const ITSM_STYLE = {
  ...
  CPI:        { bg: 'var(--sap-info-soft)', color: 'var(--sap-blue)' },
};
...
function normalizeTicket(t) {
  ...
  return {
    id: t.id,
    displayId: t.ticketNumber || t.id,
    system: sys,
    incident: t.incidentId || t.incident || t.ticketNumber || t.id,
    severity: sev,
    artifact: t.artifact || t.iflow || t.interface || t.title || 'Integration Flow',
    category: t.category ? String(t.category).replace(/_/g, ' ') : 'General',
    created: t.createdAt ? new Date(t.createdAt).toLocaleDateString() : (t.created || '—'),
    status: statusDisplay,
    assignedTo: t.assignedTo || t.assignedTeam || 'Integration Ops',
    sla: t.sla || 'Met',
    lastUpdate: t.updatedAt || t.createdAt ? new Date(t.updatedAt || t.createdAt).toLocaleDateString() : (t.lastUpdate || '—'),
  };
}
...
export default function TicketsPage({ onOpen }) {
  ...
  const normalizedList = tickets.map(normalizeTicket);

  const list = normalizedList.filter(t =>
        ...
        {ITSM_SYSTEMS.map(s => (
          ...
          >
            ...
            <span style={{ marginLeft: 5, fontSize: 11, fontWeight: 700 }}>
              {s === 'All' ? normalizedList.length : normalizedList.filter(t => t.system === s).length}
            </span>
          </button>
        ))}
